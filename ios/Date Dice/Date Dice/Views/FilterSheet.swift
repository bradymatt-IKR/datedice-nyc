        ...
        NavigationStack {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 24) {
                    ...
                    // 3. Weather — single-select
                    filterSection(
                        "\u{1F324}\u{FE0F} Weather",
                        options: AppConstants.weathers,
                        selected: vm.filters.weather
                    ) { option in
                        vm.filters.weather = vm.filters.weather == option ? nil : option
                    }
            ...
            .onChange(of: showNeighborhoods) { _, expanded in
                if expanded {
                    selectedDetent = .large
                    Task { @MainActor in
                        try? await Task.sleep(for: .milliseconds(100))
                        withAnimation { scrollProxy.scrollTo("neighborhoods", anchor: .top) }
                    }
                }
            }
            } // ScrollViewReader
